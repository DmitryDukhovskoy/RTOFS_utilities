...
class PROF1D():
  """
    Save 1D T or S profiles
  """
  def __init__(self, ZZi, AA):
    prof1d      = np.expand_dims(AA, axis=0)
    self.prof1d = prof1d
    self.zz     = ZZi

  def add_array(self, prof1d):
    prof1d      = np.expand_dims(prof1d, axis=0)
    self.prof1d = np.append(self.prof1d, prof1d, axis=0)
...
def interp_prof2zlev(AA, ZZ, ZZi, fill_deep=False, fill_surf=False, check_mincr=True):
  """
    ...
    onto ZZi 
    no nans or missing data
    if not fill_deep: then for values ZZi < ZZ[-1] missing AA are filled with nans
...
# Add extra point at the bottom for interp deep z levels:
  if ZZ0[-1] > ZZi[-1]:
    ZZ0 = np.append(ZZ0, ZZi[-1])
    AA  = np.append(AA, AA[-1])
  
  nintp = len(ZZi)
  AAi = np.zeros((nintp))
  for kk in range(nintp):
#    print(f'kk={kk} zzi={ZZi[kk]}')
    aai = minterp.pcws_lagr1(ZZ0, AA, ZZi[kk])
    AAi[kk] = aai
#
# Return nans for missing values if needed:
  if not fill_surf:
#    iz = max(np.where(ZZi > ZZ[0])[0])
#    AAi[:iz+1] = np.nan
    AAi[np.where(ZZi > ZZ[0])] = np.nan

  if not fill_deep:
#    iz = min(np.where(ZZi < ZZ[-1])[0])
#    AAi[iz:] = np.nan
    AAi[np.where(ZZi < ZZ[-1])] = np.nan
