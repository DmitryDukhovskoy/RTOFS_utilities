...
def find_gridpnts_box(x0, y0, LON, LAT, dhstep=0.5, skip_bndry=True):
  """
...
#  jmin, imin = np.where(DD == np.min(DD))
  ...
  AI = np.array([xv1mc,yv1mc])
  BI = np.array([xv1pc,yv1pc])
  C = np.array([x0c,y0c])
...
#  # Find 3rd pnt checking orientation alongJ-axis wrt I-axis
#  C = np.array([xv1jc,yv1jc])
#  orntJm1 = np.sign(mmisc1.orientation(AI,BI,C)) 
#  C = np.array([xv1lc,yv1lc])
#  orntJp1 = np.sign(mmisc1.orientation(AI,BI,C)) 
#  if orntJm1 == 0 or orntJm1 == orntI:
#    xv3 = xv1j
#    yv3 = yv1j
#  elif orntJp1 == 0 or orntJp1 == orntI:
#    xv3 = xv1l
#    yv3 = yv1l
#  else:
#    print(f"ERR: Could not find closest pnt 2 wrt to I-AXIS orientation")
#    raise Exception(f"x0={x0}, y0={y0}")
