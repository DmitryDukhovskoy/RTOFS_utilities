...
parser.add_argument("--derive", help=">0: Derive from daily, =0:  read ice_month ", default=1, type=int)
...
if args.derive is not None:
  derive_mean = bool(args.derive)  # 0 --> False
...
if jday_plt >0 and jday_plt <=366:
  ...
  dnmbR  = mtime.datenum([yr_plt,mo_plt,15])  # day to plot
...
print(f'Test  Run: {test_name} Monthly mean Plot date: {dvR[0]}/{dvR[1]}')
...
try:
  ...
  btx = 'plot_seaice_test_mnthly.py'
