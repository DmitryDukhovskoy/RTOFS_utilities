"""
  Interpolate monthly SPEAR ice conc and thickn. 
  onto NEP grid

  Saved months - f/cast months, not calendar
 
  usage: 
  interp_SPEAR_ice_mnth.py --varnm iconc --YRS 2010 --YRE 2014 --MMI=1 --ensmb 1
...
for YRI in range(YRS,YRE+1):
  ...
  for itime in range(12):
    MMF = itime+1
    print(f'Processing f/cast {YRI} month={MMF}')
  ...
  dset.attrs.update({
    "info": "Interpolated from SPEAR seasonal forecasts to NEP SIS2 grid",
    "code": "interp_SPEAR_ice_mnth.py"
  })

  if f_save:
    pthpkl = '/work/Dmitry.Dukhovskoy/anls_output/spear_ice'
    flanom = f'spear_interpNEP_{varnm}_mnth_{YRI}{MMI:02d}.nc'
    dflanom = os.path.join(pthpkl,flanom)
    print(f'Dumping {varnm}  --> {dflanom}')
