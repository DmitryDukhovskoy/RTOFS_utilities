"""
  Plot monthly ice conce from NEP BGC spinup and hindcasts
...
parser.add_argument("--expt", help="spinup, hindcast", default='hcast', type=str) 
...
for YRA in (YAVRG):
  ...
  for MMA in range(MMS,MME+1):
    ...
    # YYYYMM01.ice_month.nc
      if MMA < 4:
        flhcast = f'{YRA}0101.ice_month.nc'
        MINIT=1
      elif MMA > 3 and MMA < 7:
        flhcast = f'{YRA}0401.ice_month.nc'
        MINIT=4
      elif MMA > 6 and MMA < 10:
        flhcast = f'{YRA}0701.ice_month.nc'
        MINIT=7
      elif MMA > 9:
        flhcast = f'{YRA}1001.ice_month.nc'
        MINIT=10
      dcice = os.path.join(pthoutp,flhcast)
...
sttl = (f'{expt_name} IceConc {YRS} mo={MMS}')
...
btx = 'plot_BGChndcst_iconc_mnthly.py'
