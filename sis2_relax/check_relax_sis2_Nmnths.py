...
parser.add_argument("--yrs", help="Year to plot", type=int, required=True)
...
YRS  = args.yrs if args.yrs else None  # year to plot
...
YRE = YRS+Nyrs_file-1
flout = f'PIOMASv21_ithkn_iconc_{YRS}_{YRE}_monthly.nc'
...
sinfo = f'Monthly Rlx fields from PIOMAS \n' + diclim

iplt = 0
for MM in range(MMS,MME+1):
  dnmb0 = mtime.datenum([YRS,MM,15,12])
  D = abs(TM-dnmb0)
  itime = np.argmin(D)
  dv0 = mtime.datevec(TM[itime])
  assert dv0[0]==YRS, f'Requested YR={YRS}, year in rlx file={dv0[0]}'
...
ax3.axis('off')
btx = 'check_relax_sis2_Nmnths.py'
bottom_text(btx)
