"""
  ...
  usage: corr_iconc_SPEAR_NSIDC.py --ensmb 1 --MMI 1 --YRS 1993 --YRE 2020 --MMS 1 --MME 3
...
for YR in range(YRS,YRE+1):
  ...
  dclim_spear = msisrlx.read_SPEAR_iconc_clim_interp(YR, MMI, ens_nmb)
  ...
  for MM in cal_months:
    ...
    Clim_spear = dclim_spear['siconc'].isel(months=itime).data
    ...
    # Anomalies:
    dIspear.append(CIspear-Clim_spear)
...
sinfo = sinfo + f'SPEAR MI{MMI:02d}e{ens_nmb:-2d} seas. f/casts vs NSIDC NRT v3 ice conc'
...
sttl = f'Correlation & significance {alf:.2f} SPEAR MI:{MMI:02d}e{ens_nmb:-2d} vs NSIDC iconc anom\n'
