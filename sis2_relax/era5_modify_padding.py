"""
  ...
  Replace the day following the model blow up by interpolating 
  the previous day into all hrly slots 
  day0 --> day1(replaced) --> day2 (following day)

  if day1 is the last day, then simply copy day0 to day1
...
#                 to replace
#
# Simple linear interpolation is applied, for dltHR> several hours
# a high-order polynomial may be needed
#
...
for varnm in VARS:
  ...
  # Read next date:
  Anext, TMn, _ = read_ERA5_field(varnm, dnmb_next)

  jdm,idm = Aprev.shape
  Y2D = np.zeros((2,jdm,idm))

  TP  = np.array([dnmb_prev,dnmb_next])
  Y2D[0,:,:] = Aprev
  Y2D[1,:,:] = Anext
  ...
  for it in range(len(Tint)):
    ...
    print(f'Interpolating data to: {yri}/{mmi}/{ddi}:{hri:02d}')  
 
    A2Di = mint1d.lininterp_2D_intime(TP,Y2D,dnmbI)

    # Replace with interpolated field:
    DT = abs(TMp-dnmbI)
    itime = np.argmin(DT)
    assert(DT[itime]<1./24.),f'Time differes > 1hr, {dnmbI:.5f} vs {TMp[itime]:.5f}' 
    # CHeck introduced difference:
    D2 = darray.data[itime,:,:]-A2Di
    rmax = np.max(D2)
    rmin = np.min(D2)
    print(f"min/max error after interpolation = {rmin:.4f}/{rmax:.4f}")
    darray.data[itime,:,:] = A2Di

  dmm2 = darray.data[:,jch,ich].copy()
  dset[varnm] = darray
  dset.attrs["info"]=f"Modified {yr_ch}/{mm_ch}/{dd_ch}:0hr -/+ {dltHR}hrs interpolated"
  dset.attrs["code"]="/home/Dmitry.Dukhovskoy/python/sis2_relax/era5_modify_padding.py"

  if f_save:
    flnew = f'ERA5_{varnm}_{YR}_intrp{int(dltHR):03d}hrs_padded.nc'
