"""
  ...
  model trained in
  train_HistGradBoostRegr_ithkn.py
...
parser.add_argument("--gbrmod", help="Hist Gradient Boosting decision tree model number",
...
# Load model parameters and GradBoost model:
pthgbr = config_predictor["linregr"]["pthgbr"]
model_file = os.path.join(pthgbr, model_name + ".pkl")
print(f"Reading HistGradBoost object from {model_file}")
gbr = joblib.load(model_file)

# Training period:
info_file = os.path.join(pthgbr, model_name + "_info.json")
...
# GradBoost Predictors:
pred_file = os.path.join(pthgbr, model_name + "_predictors_trainidx.npz")
