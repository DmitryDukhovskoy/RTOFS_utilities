"""
  Subset 2D fields from cobalt omip_sfc 
  dissicos
  phos
  o2os
  chlos
...
VAROCN = ['dissicos','phos','o2os','chlos','co3os','co3sataragos']
...
for YR in range(YRS,YRE+1):
  ...
  for MM in MNTHS:
    ...
    for ens in ENSMB:
      ...
      dflin = os.path.join(pthfcst,'ocean_cobalt_omip_sfc.nc')
      flout = f'ocean_cobalt_omip_sfc_{subdir}.nc'
      dflout = os.path.join(pathout,flout)

      print(f'\nsubsetting {dflin} --> {dflout}')
      ...
      for vars in VARDROP:
        ...
        ds.attrs["code"] = f"/home/Dmitry.Dukhovskoy/python/setup_seasonal_NEP/subset_cobalt_omip_sfc.py"
