...
parser.add_argument("--MMS", help="Cal. month start to plot", type=int, required=True)
parser.add_argument("--MME", help="Cal. month end to plot", type=int)
...
nmnths = MME-MMS+1
ncol = args.ncol if args.ncol else 1
...
def plot_field(ax1, fig1, m, xR, yR, A2d, clrmp, rmin, rmax, plt_clrb, sttl=[]):
  ...
  # extend: min, max, both
  if plt_clrb: 
    ax2 = fig1.add_axes([0.965,0.1,0.012,0.8])
...
plt.ion()
plt.close('all')
fig1, axes = plt.subplots(nrow, ncol, figsize=(12, 8))  # 3 rows, 4 columns
...
for MMA in range(MMS,MME+1):
  ...
  if YRS==YRE:
    sttl = f'IceConc {YRS} {MMA:02d}'
  else:
    sttl = (f'IceConc avrg {YRS}-{YRE} {MMA:02d}')
  ...
  if f_cntrobs:
    ax1 = contour_nsidc(ax1,YAVRG,MMA,xR,yR,cntr_nsidc)
  if f_cntrrlx:
    ax1 = contour_piomas_irlx(ax1,xR,yR,YAVRG,MMA,cntr_irlx,HH)

plt.tight_layout()
ax3 = plt.axes([0.02, 0.02, 0.1, 0.1])
dx = 0.15
dy = 0.08
yy0 = 0.1
...
btx = 'plot_hcast_iconc_mnthly_Nsubplots.py'
