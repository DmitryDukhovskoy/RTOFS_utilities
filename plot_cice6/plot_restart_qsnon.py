"""
  Plot CICE6 3D snow enthalpy[ncat, ny, nx]
  from CICE6 restart
"""
import os
import numpy as np
...
btx = 'plot_restart_qsnon.py'
rmin   = -140.  # MJ
rmax   = -110.  # max enthalpy to give T snow <= 0C
...
stl = 'CICE6, {0}/{1}/{2}, {3} J/m^3*{4}, icat={5}, lyr={6}\n {7}'.\
        format(YR,MM,MD,fld,cff,kcat+1,ilyr,fl_restart6)
...
for kcat in range(5):
  ...
  print('{0}, i={5}, j={6}, Cat: {1}, Lr: {2}, max enth={3}, Temp={4}'.\
        format(fld, kcat+1, lsnow, qsn, zTsn,i0,j0))
