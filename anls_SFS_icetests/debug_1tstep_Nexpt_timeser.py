"""
  ...
  plot N experiments at 1 grid point
...
parser.add_argument("--enmb", 
    help=f"experiment number: 0, 1, 2, ...", 
    type=int,
    nargs="+",       
)
...
# Rapid snow change:
IJp = [303, 1062]     # rapid snow change point
#IJp = [311, 1070]      # slow snow change
IJp = np.array(IJp)
...
A2d = read_2Dcice(fday2, nsec2, HH, init_date, init_hr, ENMBS[0], varnm0)

# Derive time series:
npp = len(ENMBS)
nrec = len(RECT)
VALS = np.zeros((nrec,npp)) * np.nan
for ipp, enmb in enumerate(ENMBS):
  ii0, jj0 = IJp
  print(f"Processing enmb={enmb}")
...
plt.clf()
ax1 = plt.axes([0.08, 0.6, 0.8, 0.33])
...
ax3 = fig1.add_axes([0.08, 0.14, 0.9, 0.4])
LBLS = []
for ipp in range(ENMBS.shape[0]):
  enmb = ENMBS[ipp]
  clr0 = CLRS[ipp]
  ii0, jj0 = IJp
  line_lbl = mgfscice.sfs_tests_info(enmb)
  ln1, = ax3.plot(VALS[:,ipp], '-', linewidth=2, color=clr0, label=f"expt{enmb} {line_lbl}")
  LBLS.append(ln1)

#ax3.set_xlim([0.15, 1.05])
ax3.grid('on')

sttl3 = f"{varnm0} ({units}) {lname}, i={ii0} j={jj0}" 
ax3.set_title(sttl3)
...
btx = 'debug_1step_Nexpt_timeser.py'
