"""
  Plot melt pond fraction of sea ice (!) 
  or melt pond depth over sea ice
  daily fields 
  SFS runs
"""
import os
...
# apond, hpond - pond area (fraction of ice) and depth
#                hpond = pndaspect * apond, hpond * apond = vpond
# apeff - effective pond area used for radiation calculation 
#         effective pond area fraction seen by the shortwave/albedo parameterization
...
match fld_name:
  case 'apond':
    varnm = 'apond_d'
    lname = 'melt pond fract of seaice'
    units = 'fraction'
    rmin = 0
    rmax = 1
    clrmp = mclrmps.colormap_warm(CLRMP=['Blues','Wistia'])
  case 'apeff':
    varnm = 'apeff_d'
    lname = 'effective pond area for albedo'
    units = 'fraction'
    rmin = 0
    rmax = 1
    clrmp = mclrmps.colormap_warm(CLRMP=['Blues','Wistia'])
  case 'hpond':
    varnm = 'hpond_d'
    lname = 'melt pond depth'
    units = 'm'
    rmin = 0
    rmax = 0.2
    clrmp = mclrmps.colormap_warm(CLRMP=['GnBu'])
...
sttl = f"{lname} {units}, SFS expt{enmb:02d} ({expt_name}) \ninit {init_date} FDAY={fday:02d}, {YR}/{MM:02d}/{DD:02d}"

sinfo = f'melt pond daily fields, {varnm}\n'
sinfo = sinfo + dflice
