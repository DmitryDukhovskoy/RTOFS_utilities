"""
  Compare freeze/melt potential
  if >0, new ice forms; if <0, ice melts
  W/m2

  Initial freeze/melt potential is = 0
  no need to plot
"""
import os
import numpy as np
...
for enmb in ENMBS:
  ...
  for nn in range(nrecs):
    ...
    with xarray.open_dataset(dflcice) as dcice:
      ...
      FRZ = dcice['frzmlt_d'].values.squeeze() * 1.e-3  # W --> kW
...
ax1.set_title(f'Freezemelt potential (kW/m2), SFS init {init_date}, regn={regn}')
