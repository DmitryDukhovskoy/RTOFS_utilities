"""
  Plot diffenece maps of MOM6 pot temp
  btw 2 experiments
  SFS runs
...
parser.add_argument("--enmb", 
                help="List 2 experiment numbers: e.g. 1, 6", 
                nargs="+",
...
def read_mom6(dflocean, ilr):
  print(f"Processing {dflocean}...")
...
enmb2 = ENMBS[1]
if plot_init:
  pthoutp1 = '/gpfs/f6/sfs-emc/proj-shared/Dmitry.Dukhovskoy/RUNDIRS/restart_sfs_C192mx025/ocean'
  pthoutp2 = pthoutp1
  ...
  pthoutp1 = f"/gpfs/f6/sfs-cpu/scratch/Dmitry.Dukhovskoy/sfs_C192mx025_cice_test/expt{enmb1:02d}/mom6"
  pthoutp2 = f"/gpfs/f6/sfs-cpu/scratch/Dmitry.Dukhovskoy/sfs_C192mx025_cice_test/expt{enmb2:02d}/mom6"
  flinp = f"oceanm_{YRh}_{MMh:02d}_{DDh:02d}.nc"

dflocean1 = os.path.join(pthoutp1,flinp)
dflocean2 = os.path.join(pthoutp2,flinp)

AA1 = read_mom6(dflocean1, ilr)
AA2 = read_mom6(dflocean2, ilr)
A2d = AA2 - AA1
...
test_info1 = mgfscice.sfs_tests_info(enmb1)
test_info2 = mgfscice.sfs_tests_info(enmb2)

sttl = f"MOM6 lr={lr} diff(potT): expt({test_info2})-expt({test_info1}),  init {init_date}, FDAY={fday:02d}\n "
...
sinfo = sinfo + dflocean1
sinfo = sinfo + dflocean2
