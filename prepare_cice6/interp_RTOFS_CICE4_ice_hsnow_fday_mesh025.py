"""
  Interpolate RTOFS CICE4 fields for f/cast day fday
...
parser.add_argument("--field", help=f"Field to interpolate",
                    choices=['hsnow','ithkn','iconc'], required=True, type=str)
parser.add_argument("--init", help="RTOFS init date", 
                    choices=[20250704], required=True, type=int)
parser.add_argument(
  "--fhr", help="List of forecast hours to process",
  choices=[0,24,48,72,96,120,144,168,192],
...
init_date = args.init 
FHOURS    = args.fhr  # RTOFS forecast hours
...
def write_nc(A2d, time_out, field_name, dfliceout, varnm, rtofs_vers):
  ...
  dset.attrs['source']      = 'interp_RTOFS_CICE4_ice_hsnow_fday_mesh025.py'
...
for fhr in FHOURS:
  print(f"\nInterpolating {field_name} RTOFS CICE4 {init_date} fcast hr={fhr} --> mesh025\n")
  ...
  fliceout = f"{field_name}_RTOFSv{rtofs_vers}_{init_date}_fhr{fhr:03d}_{jdm}x{idm}.nc"
