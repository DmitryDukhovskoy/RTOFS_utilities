"""
  Derive daily clim of snow thickness in Antarctica 
  derived from SSM/I and AMSR 19 and 37 GHz microwave brightness temperatures
  https://earth.gsfc.nasa.gov/cryo/data/antarctic-snow-depth-sea-ice
  available data: 1992-2007
  daily data
"""
import os
import numpy as np
...
parser.add_argument("--yrS", help="Start year to average, 1992,..., 2007", type=int, required=True)
...
match node_nm:
  ...
  case 'gaea' | 'dtn':
    ...
    pthout_snow = '/gpfs/f6/sfs-cpu/scratch/Dmitry.Dukhovskoy/data/snow_nasa/daily_clim'
...
for MM in range (moS,moE+1):
  ...
  for jday in range(jd1,jd2+1):
    ...
    print(f'Processing {MM}/{jday} jday1={jd1} jday2={jd2}')
    icc = 0
    ASUM = None
    ...
    if icc > 1:
      print(f"Read {icc} records for jday={jday}")
      HS2D = ASUM / float(icc)
    else:
      raise Exception(f"Found only {icc} records for month {MM} day_year={jday}")

    # Land = 200
    HS2D[HS2D>199.]=np.nan   # thickns in cm !, 200 - land
    
    mday = jday-jd1+1
    print(f'{MM:02d}/{mday:02d}, Min/max snow (cm) = {np.nanmin(HS2D):.4f}/{np.nanmax(HS2D):.4f}')

    if H3D is None:
      jdim, idim = HS2D.shape
      mo_days = jd2-jd1+1
      H3D = np.zeros((mo_days,jdim,idim))
  ...
  # Save to netCDF:
  time_days = np.arange(1,mo_days+1, dtype='int32')
  ...
  dset_hs.attrs.update({
    ...
    "source": "derive_hsnow_dailyclim_SSMI_Antarctic.py",
    "contact": "dmitry.dukhovskoy@noaa.gov",
    "region": regn,
    "Grid": "Southern Hemisphere Stereographic Polar Projection Based on WGS 1984"
  })

  floutp = f"SSMI_Antarctic_hsnow_daily_clim_{MM:02d}_{idim}x{jdim}.nc"
  dflout = os.path.join(pthout_snow,floutp)
  print(f"Saving --->   {dflout}")
  dset_hs.to_netcdf(dflout, 
...
if f_chck:
  ...
  mday = 2
  sttl=f'hsnow {MM:02d}/{mday:02d}, {yrS}-{yrE}'

  hs2d = H3D[mday-1,:,:].squeeze()
  ...
  btx='derive_hsnow_dailyclim_SSMI_Antarctic.py'
