"""
  Interpolate GFSv16 --> GFSv17 mesh025 MOM6/CICE6 grid
  Both polar regions are interpolated

  Produce averaged or instant. fields

  gmapi indices: get_gmapi_gfs16_reggrid_to_mesh025.py
...
ftmp = 0       # temporary files with interp fields, saved every averaging period
init_date = 20251231
init_hr = 0
#FH = 0    # forecast hours
regn = 'global'
...
parser.add_argument("--ftmp", help=f"Save tmp interp. monthly fields, default={ftmp}", 
...
ftmp = args.ftmp if args.ftmp is not None else ftmp
...
fgmapi  = 'GFSv16_reggrid2mesh025_gmapi_1440x1080_SNpoles.nc'
...
match field_name:
  case 'hsnow':
    varnm = 'SNOD_surface'
    attr_str_long = 'snow depth'
    attr_units = 'm'
  case 'ithkn':
    varnm = 'ICETK_surface'
    attr_str_long = 'ice thickness'
    attr_units = 'm'
  case 'iconc':
    varnm = 'ICEC_surface'
  ...
  case 'snwed':
    varnm = 'WEASD_surface'   # water equiv. of accum snow depth kg/m2  
    attr_str_long = 'water equivalent of accumulated snow depth'
    attr_units = 'kg/m2'
  case 'albd':
    varnm = 'ALBDO_surface'   # prcnt
    attr_str_long = 'snow/ice surface albedo'
    attr_units = 'reflected fraction'
  case _:
...
# GFSv17: /gpfs/f6/gfs-cpu/world-shared/Lydia.B.Stefanova/forDmitry/GFSv17_icesubset/gfs.20251231/00/model/ice/history
pthgfs  = f'/gpfs/f6/gfs-cpu/world-shared/Lydia.B.Stefanova/forDmitry/GFSv16_icesubset/{init_date}'

# Intepolated fields:
pthint = f"/gpfs/f6/sfs-emc/proj-shared/Dmitry.Dukhovskoy/GFSv16/{init_date}/intrp_mesh025"
pthtmp = os.path.join(pthint,f'tmp_{field_name}')
os.makedirs(pthtmp, exist_ok=True)
flint  = f"GFSv16_{field_name}_{init_date}_advrg{hrav}hr_{regn}.nc"
...
TM = []
for FH in TMF:
  flgfs = f'gfsv16_icesubset.{init_date}.fh{FH:03d}.nc'
  dflgfs = os.path.join(pthgfs,flgfs)
  print(f"Reading GFSv16: {dflgfs}")
  ...
  if irec_avrg == nrec_avrg:
    ...
    ftmp = f"gfsv16_{field_name}_{init_date}_{regn}_day{iday}.npy"
    dfltmp = os.path.join(pthtmp, ftmp)
    if save_tmp and os.path.exists(dfltmp):
      # Check if tmp file exists then load and skip interpolation:
      print(f"Skipping {YR}/{MM}/{DD:02d} lead={iday}: already computed <--- {dfltmp}\n")
      AI = np.load(dfltmp)
    else:  
      HSint = msisrlx.interp2Dfld(A2d, IMOM, JMOM, INDX, JNDX, LMsk, LON, LAT, hlon, hlat)
      if save_tmp:
        print(f"Dumping tmp file -->  {dfltmp}")
        np.save(dfltmp, HSint)
      AI = np.where(HH >=0, np.nan, HSint)
...
dset.attrs['title']       = f'GFSv16 forecast {attr_str_long} daily averaged interpolated onto mesh025' 
dset.attrs['institution'] = 'NOAA NWS NCEP MDC'
dset.attrs['source']      = 'interp_GFSv16_iceflds_mesh025.py'
...
dayS = fhrS // 24
dayE = fhrE // 24
flout = f"gfsv16_{field_name}_init{init_date}_days{dayS:03d}_{dayE:03d}.nc"
dfliceout = os.path.join(pthint,flout)
...
if f_chck:
  ...
  sttl = f"{field_name} GFSv16 interp mesh025 \n{flgfs}"
  ...
  btx = 'interp_GFSv16_iceflds_mesh025.py'
  bottom_text(btx) 
