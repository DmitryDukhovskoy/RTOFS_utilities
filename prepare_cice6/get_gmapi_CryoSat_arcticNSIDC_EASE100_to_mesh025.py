...
LON[LAT < -100] = np.nan  # out of range values 
LAT[LAT < -100] = np.nan
...
if ignore_north_lim:
  print(f"WARN: Indices north of northernmost lat={np.nanmax(LAT):.2f} will be searched")
else:
  print(f"WARN: Interpolation performed up to max lat={np.nanmax(LAT):.2f}, if <90 - possible discontinuities ...")
...
for ii in range(idm):
  ...
  for jj in range(jS,jE+1):
    ...
    ixx, jxx = mrmom.find_gridpnts_box(x0, y0, LON, LAT, dhstep=5., ignore_north_lim=ignore_north_lim)
...
# Global attributes
dset.attrs['title']       = 'Grid mapping between Arctic polar sterographic EASE100 and MOM6 mesh025 grids'
dset.attrs['institution'] = 'NOAA NWS NCEP MDC'
dset.attrs['source']      = 'get_gmapi_CryoSat_arcticNSIDC_EASE100_to_mesh025.py'
dset.attrs['history']     = 'Arctic sea ice thickness from CryoSat-2 monthly clim'
dset.attrs['contact']     = 'dmitry.dukhovskoy@noaa.gov'
dset.attrs['region']      = regn

pthdump = os.path.join(pthdata,'gmapi_NSIDC')
fgmapi  = f'CryoSat_NSIDC_EASE100_MOM6_gmapi_{idm}x{jdm}_{regn}.nc'
...
if f_chck:
  ...
  btx = 'get_gmapi_CryoSat_arcticNSIDC_EASE100_to_mesh025.py'
  bottom_text(btx) 
