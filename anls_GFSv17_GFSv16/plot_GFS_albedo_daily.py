...
match gfsv:
  case 16:
    ...
    flgfs = f'gfsv16_albd_init{init_date}_days000_016.nc'
    varnm = 'ALBDO_surface'
    cff = 0.01   # 100% --> 1
  case 17:
    pthgfs = f'/gpfs/f6/sfs-emc/proj-shared/Dmitry.Dukhovskoy/GFSv17/{init_date}/daily'
    flgfs = f'gfsv17_albd_init{init_date}_days000_016.nc'
    varnm = 'albsni_h'
    cff = 0.01
...
with xarray.open_dataset(dflice) as dcice:
  A2d  = cff * dcice[varnm].isel(time=fday-1).data.squeeze()
...
sttl = f"albedo GFSv{gfsv} init {init_date}, lead time={fday:02d} days\n {YR}/{MM:02d}/{DD:02d}"

sinfo = 'daily average surface albedo from CICE6\n'
sinfo = sinfo + dflice
