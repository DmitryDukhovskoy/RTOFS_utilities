"""
  Salinity at depth level lr0 in the Bering Sea region of the NEP domain
  stereogr proj
"""
import os
import numpy as np
...
ZM   = dset['zl'].data 
SS   = dset['salt'].isel(time=0, zl=lr0-1).data
zm0  = ZM[lr0-1]
...
ss1 = f'{runname} salin at -{abs(zm0):.0f} m \n'
sinfo = ss1 
...
img = m.pcolormesh(xR, yR, SS, cmap=clrmp, vmin=rmin, vmax=rmax)
...
sttl = f"{runname} salin -{abs(zm0):.0f} m, avrg: {yrs}/{mms}/{dds}-{yre}/{mme}/{dde}"
