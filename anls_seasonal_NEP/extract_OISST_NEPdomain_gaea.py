The code does not work with OpenDap netcdf
copy netcdf to PPAN and use
extract_OISST_NEPdomain.py 
on PPAN to subset the domain
...
# OpenDap to PSL data does not work on PPAN but works on Gaea
...
print(f'Subsetting {YR} {furl}')
# There is a bug in netcdf and whole slice cannot be read in
# read by 1 day at a time
...
for iday in range(365):
  ...
  dstr = f'{dv[0]}-{dv[1]:02d}-{dv[2]:02d}'
  ds_sst = xarray.open_dataset(furl).sel(time=slice(dstr))
  Tdnmb.append(dnmb)
  T2d    = ds_sst['sst'].data[jj1:jj2+1, ii1:ii2+1].squeeze()
  if iday == 0:
    SST = T2d.copy()
  else:
    SST = np.append(SST, T2d, axis=0)


pthsst = '/gpfs/f5/cefi/scratch/Dmitry.Dukhovskoy/tmp_anls/OISST' 
...
# xarray gives wierd error, cannot process the whole array
...
dset_sub.attrs['code'] = '/home/Dmitry.Dukhovskoy/python/setup_seasonal_NEP/extract_OISST_NEPdomain_gaea.py'
