"""
  Plot monthly ice thkn from NEP physics hindcast with GLORYS nudging
...
# Default values: 
trlx = 0   # max relax. time scale, hrs
expt = 'glorys_nudging'
...
run_info = f'{expt_name}  ice thkn: {min(YAVRG)}-{max(YAVRG)} months={MMS}-{MME}'
...
for YRA in YAVRG:
  for MMA in range(MMS,MME+1):
    ...
    pthhcst = '/archive/Dmitry.Dukhovskoy/fre/NEP/2024/NEP_physics_202404_nudging-15d/' +\
    ...
    H2d = ds['sithick'].isel(time=imo).data.squeeze()   # ice thickness m, need m3/m2 
...
def plot_field(fgnmb, m, xR, yR, A2d, clrmp, rmin, rmax, sttl=[]):
  ...
  btx = 'plot_NEPphys_hcast_ithkn_mnthly.py'
