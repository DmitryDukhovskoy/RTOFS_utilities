...
TCLASS = [-2., -1., 0., 1., 2.]
nTC    = len(TCLASS)
CPA    = np.zeros((4,nTC-1))   # Cold pool area: 4 seasons x T classes
icc    = 0
for MM in range(1,12,3):
...
if f_save:
  pthanls = pthseas['MOM6_NEP']['seasonal_daily']['pthanls'].format(expt_nmb=2)
  dflout  = os.path.join(pthanls,f'Bering_coldpoolarea_WOA23_{yr1_dec}-{yr2_dec}.pkl')
  print(f'Saving WOA23 coldpool area --> {dflout}')
...
sttl = f"WOA23 coldpool area x {cff} km2 ,  decade:{yr1_dec}-{yr2_dec}"
...
for icl in range(nTC-1):
  ...
  for ii in range(4):
    ...
    x2 = Xtime[ii+1]
    y1 = CPA[ii,icl]
    ln1, = ax1.plot([x1,x2], [y1,y1], linewidth=2, color=clr0, label=tline)
    if ii < 3:
      y2 = CPA[ii+1,icl]
      ax1.plot([x2,x2], [y1,y2], linewidth=2, color=clr0)
  LNS.append(ln1)
#
...
for ii in range(4):
  ...
  x2 = Xtime[ii+1]
  y1 = CPT[ii]
  ln1, = ax1.plot([x1,x2], [y1,y1], linewidth=2, color=clr0, label='Total')
  if ii < 3:
    y2 = CPT[ii+1]
    ax1.plot([x2,x2], [y1,y2], linewidth=2, color=clr0)
LNS.append(ln1)
...
btx = 'coldpool_area_Tclass_WOA23.py'
