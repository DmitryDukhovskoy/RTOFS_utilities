"""
  Plot snow depth vs sea level from  CICE6 restart fields
  to check that snow-ice interface stays above sea level
  if not - snow will be melted to bring the interface at sea level
"""
import os
import numpy as np
...
with xarray.open_dataset(dflrst_in) as ds_out:
  ...
  vicen = ds_out['vicen'].data

aice = np.nansum(aicen, axis=0).squeeze()  # aggreagetd ice conc
vsno = np.nansum(vsnon, axis=0).squeeze()  # snow volume per cell m2 = cell-mean ice thickness
vice = np.nansum(vicen, axis=0).squeeze()  # ice volume per cell m2

# Ice-mean thickness of snow and ice:
hice  = np.divide(vice, aice, out=np.zeros_like(vsno), where=aice != 0)
hsnow = np.divide(vsno, aice, out=np.zeros_like(vsno), where=aice != 0)

# Ice + snow freeboard:
hfree = hice + hsnow - 1./rho_ocean * (rho_ice*hice + rhos*hsnow)

# Height of snow freeboard wrt sealevel, i.e. ice freeboard
# <0 - below sea level
Sfrb = hfree - hsnow  
...
sttl = f"Snow-ice interf. wrt sea level (m),  CICE6 {flrst_in}"
...
img1 = ax1.pcolormesh(xh, yh, Sfrb, cmap=clrmp, vmin=rmin, vmax=rmax)
# Overlay land
...
btx = 'map_cicerestart_snowsealvl.py'
