"""
  Spatial maps of snow/ice surface T
"""
import os
import numpy as np
...
with xarray.open_dataset(dflice) as dcice:
  ...
  AA = dcice['Tsfc_d'].data.squeeze()   # snow/ice surface temperature

AA = np.where(RMsk == 0, np.nan, AA)
AA = np.where(aice < 1.e-11, np.nan, AA)
...
rmin = -3.
rmax = 0.
clrmp.set_bad(color=[1.0, 1.0, 1.0])
...
if plot_init:
  ax1.set_title(f'UFS expt{enmb:02d}-init snow/ice Tsurf {YR}/{MM:02d}/{DD:02d}')
else:
  ax1.set_title(f'UFS expt{enmb:02d} snow/ice Tsurf {YR}/{MM:02d}/{DD:02d}')
