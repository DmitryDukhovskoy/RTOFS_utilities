"""
  Plot atm/ice stress maps for Arctic / S. Ocean
...
with xarray.open_dataset(dflice) as dcice:
  UU = dcice[varnm_u].data.squeeze()
  VV = dcice[varnm_v].data.squeeze()

UU = np.where(HH >= 0, np.nan, UU)
VV = np.where(HH >= 0, np.nan, VV)

Tau = np.sqrt(UU**2 + VV**2)
...
img1 = ax1.pcolormesh(xh, yh, Tau, cmap=clrmp, vmin=rmin, vmax=rmax)

ax1.set_title(f'{expt} init {init_date}/{init_hr:02d}, |atm/ice stress| N/m2, '+\
              f'fcast {fhr}hr  {YR}/{MM:02d}/{DD:02d} {hr0:02d}')
