"""
  Check daily clim hsnow fields
  interpolated on mesh025 grid

  derived from NASA SSM/I AMSR-E snow depth data
  1992-2007 

  see: interp_SSMI_hsnow_dailyclim_antarct_mesh025.py
...
parser.add_argument("--mm", help="month to plot 1,...,12", type=int, required=True)
parser.add_argument("--dd", help="month day to plot 1,...{28,30,31}", type=int, required=True)
...
# daily snow depth, Antarctica, original grid:
pthdata = pths_ufs[node_nm]["MOM6"]["pthdata"]
pthsnow = os.path.join(pthdata,'snow_nasa','daily_clim')
flsnow = f"SSMI_Antarctic_hsnow_daily_clim_{MM:02d}_316x332.nc"
...
with xarray.open_dataset(dflsnow) as ds_snow:
  ...
  HS = ds_snow['snow_depth'].isel(time=DD-1).data.squeeze()  # cm

# Interpolated fields:
flsnow_int = f'SSMI_hsnow_mesh025_1440x1080_dailyclim_{MM:02d}_south.nc'
dflsnow_int = os.path.join(pthsnow,flsnow_int)
print(f"Reading interpolated hsnow {dflsnow_int}")
with xarray.open_dataset(dflsnow_int) as ds_intrp:
  HSi = ds_intrp['snow_depth'].isel(time=DD-1).data.squeeze() *100. # m ---> cm

sinfo = f"NASA grid: {dflsnow}\n"
sinfo = sinfo + f"mesh025: {dflsnow_int}"
...
ax1.set_title(f"SSM/I hsnow, cm, interp mesh025, {MM:02d}/{DD:02d}")
