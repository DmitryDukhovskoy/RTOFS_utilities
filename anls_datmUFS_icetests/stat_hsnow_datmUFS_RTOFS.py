"""
  Compare statistics of snow depth in datmUFS with RTOFS CICE5 IC vs 
...
flname = f'hsnow_RTOFSv{rtofs_vers}_{init_date}_fhr{fhr:03d}_1080x1440.nc'
dflname = os.path.join(pth_rtofs, flname)
print(f"Reading RTOFS iconc: {dflname}")
with xarray.open_dataset(dflname) as dsrtofs:
  HSrtofs = dsrtofs['snow_depth'].isel(time=0).data.squeeze()
...
HSufs[~IMsk_ufs | ~IMsk_rtofs] = np.nan
HSrtofs[~IMsk_ufs | ~IMsk_rtofs] = np.nan

Msk_valid = np.isfinite(HSufs) & np.isfinite(HSrtofs)
Nvalid = np.sum(Msk_valid)

# Pre-extract valid grid points:
HSu = HSufs[Msk_valid]
HSr = HSrtofs[Msk_valid]

# Calc abs err:
abs_err = np.abs(HSu - HSr) 
diff_err = (HSu - HSr)
rmse = np.sqrt(1/Nvalid * np.sum( (HSu - HSr)**2 ))

hbin = np.append(np.arange(0, 0.4, 0.04), 100.)


# Histogram counts:
hist_ufs, _   = np.histogram(HSu, bins=hbin)
hist_rtofs, _ = np.histogram(HSr, bins=hbin)
...
for ii in range(1, nbins+1):
  ...
  if np.any(msk):
    ...
    rmse_bin[ii-1] = np.sqrt(np.mean( (HSu[msk] - HSr[msk])**2 )) 
...
sttl1 = f"hsnow, {init_date}, FDAY={YRF}/{MMF}/{DDF}, {regn}"
clr_ufs = [0.2, 0.5, 0.8]
...
sttl2 = f"RMSE hsnow RTOFS vs datmUFS, {regn}"
ax2 = plt.axes([0.57, 0.6, 0.42, 0.35])
ax2.bar(bin_cntr, rmse_bin, width=dbw, color=clr_rmse)
ax2.set_xlabel('Ice thickness (m)')
ax2.xaxis.set_ticks(hist_xticks)
ax2.grid(True, alpha=0.3)
ax2.set_title(sttl2)

sttl3 = f"Error hsnow datmUFS-RTOFS, {regn}"
ax3 = plt.axes([0.08, 0.15, 0.42, 0.35])
ax3.bar(bin_cntr, err_bin, width=dbw, color=clr_err)
...
btx = 'stat_hsnow_datmUFS_RTOFS.py'
