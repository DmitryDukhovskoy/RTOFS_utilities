"""
  Use snow depth (m) from GFSv17 forecasts
  output from atmospheric model FV3
  float snod(time, grid_yt, grid_xt) ;
    snod:_FillValue = 9.99e+20f ;
    snod:cell_methods = "time: point" ;
    snod:long_name = "surface snow depth" ;
    snod:missing_value = 9.99e+20f ;
...
for hrf in HRFCST:
  ...
  with xarray.open_dataset(dflice) as ds:
    A2d = ds[varnc].isel(time=0).squeeze().data * 100. # m --> cm
    #Rho_snow = ds['rhonewsn'].isel(time=0).squeeze().data
    if TLON is None:
      TLON = ds['lon'].data
      TLAT = ds['lat'].data
...
rmin = 0.
rmax = 20.
sinfo = 'Snonow depth (m) from GFSv17 FV3 output \n'
...
sttl = f'{varnm} {units}, GFSv17-FV3 {expt} init:{init_date}/{init_hr} fcast:{fhrS}-{fhrE}'
