...
parser.add_argument("--regn", help=f"hemisphere: north or south, default={regn}", type=str, required=True)
parser.add_argument("--lmask", help="Use ocean mask from SSH fild (1), or NaN for ocean (0)", 
                   choices=[0,1], default=1, type=int)
args = parser.parse_args()

rdate = args.rdate
regn  = args.regn
use_lmask = args.lmask == 1
...
pthice = f"/uda/Global_Ocean_Physics_Reanalysis/global/daily/siconc/{YR}"
...
with xarray.open_dataset(dflice) as dsice:
  A2d = dsice['siconc'].isel(time=0).data.squeeze()
...
ax1.set_title(f"iconc GLORYS {YR}/{MM:02d}/{DD:02d}")
