"""
  Closest neighbor rmapping
  GLORYS ---> mesh025 UFS grid
...
# Subset source grid (GLORYS) for faster search algorithm
jlat0 = None
if regn == 'north':
  jlat0 = np.where(LAT < lat0)[0][-1]
  sublat1d = LAT[jlat0:]
elif regn == 'south':
  jlat0 = np.where(LAT > lat0)[0][0]
  sublat1d = LAT[:jlat0+1]

subLON, subLAT = np.meshgrid(LON, sublat1d)

# Convert to spherical coord:
Z_GLR = np.sin(subLAT * dgr2rad)
X_GLR = np.cos(subLAT * dgr2rad) * np.cos(subLON * dgr2rad)
Y_GLR = np.cos(subLAT * dgr2rad) * np.sin(subLON * dgr2rad)
...
# Define mesh025 domain and target grid points:
if regn == 'north':
  DOMAIN = (LAT_m25 > lat0) & (LMsk_m25 == 1)
elif regn == 'south':
  DOMAIN = (LAT_m25 < lat0) & (LMsk_m25 == 1)
...
def save_netcdf(JM25, IM25, JGLR, IGLR, LONm025, LATm025, LONGLR, LATGLR, dfgmapi, regn):
  """
    NetCDF writer
  """
  # Indices on mesh025 grid where interpolation is done
  JM25 = np.asarray(JM25, dtype=int)
  IM25 = np.asarray(IM25, dtype=int)
  # GLORYS indices from where interpolation is done:
  ...
  dset['mesh025_indx'].attrs['long_name']   = 'UFS mesh 0.25-degree grid I index, to where interpolate'
  dset['mesh025_jndx'].attrs['long_name']   = 'UFS mesh 0.25-degree grid J index, to where interpolate'
  dset['glorys_indx'].attrs['long_name']    = 'GLORYS grid I index, from where interpolate'
  dset['glorys_jndx'].attrs['long_name']    = 'GLORYS grid J index, from where interpolate'
  ...
  # Global attributes:
  dset.attrs['title']    = 'Grid mapping from GLORYS to UFS 0.25-dgr grid, {regn}, closest neighbour'
  #dset.attrs['source']   = 'find_remap_indx_GLORYS_to_mesh025.py'
  dset.attrs['source']   = 'find_remap_indx_GLORYS_to_UFSmesh025.py'
...
xyz_GLR = np.column_stack((
  X_GLR.ravel(),
  Y_GLR.ravel(),
  Z_GLR.ravel()
))

tree = cKDTree(xyz_GLR)
...
for jj, ii in zip(J,I):
  ...
  jmin, imin = np.unravel_index(idx, X_GLR.shape)
  ...
  if run_debug:
    ...
    dot_tree = (
      X_GLR[jmin, imin] * x0plr +
      Y_GLR[jmin, imin] * y0plr +
      Z_GLR[jmin, imin] * z0plr
    )
  ...
  # Check final indices:
  x0GLR = (hlon[jMmin, iMmin] + 360) % 360  
  y0GLR = hlat[jMmin, iMmin]
  dlon = abs((x0GLR - x0 + 180.0) % 360.0 - 180.0)
  dlat = abs(y0GLR - y0)
  dlt_dgr = max(dlon, dlat)
  km1dgr = 111.
  dlt_km = np.sqrt((np.cos(y0GLR * dgr2rad)*km1dgr*dlon)**2 + (km1dgr*dlat)**2)
  ...
  if dlt_km > dlt_km_max:
    ...
    print(f"x0={x0}, y0={y0} Found pnt: x={x0GLR}, y={y0GLR}")
    raise RuntimeError(f"dlt_km {dlt_km} > {dlt_km_max}")

  # Regitser
  JM25.append(jj)    # mesh025
  IM25.append(ii)    # mesh025
  JGL.append(jMmin)  # GLORYS
  IGL.append(iMmin)  # GLORYS

if f_save:
  flout = f"gmapi_closenghb_GLORYS_to_UFSmesh025_{regn}.nc"
