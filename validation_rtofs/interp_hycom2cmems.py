...
if not f_save:
  print('\n  Interpolated fields will not be saved !!! \n')
...
# Check if already saved:
  ...
  if os.path.isfile(flsave):
    print('File exist ' + flsave)
...
# Global mean SSH:
  f_calc_sshmn = False
  if f_calc_sshmn:
    DX, DY = mhycom.dx_dy(XH,YH)
    Acell = DX*DY
    Acell = np.where(np.isnan(SSH),np.nan,Acell)
    Area  = np.nansum(Acell)
    ssh_mn = np.nansum(SSH*Acell)/Area
...
# Find basis functions for a reference rectangle:
  ...
  # The order of vertices should not matter for mapping
  # onto reference rectangle
    eH = SSH[jH,iH]
    xH = XH[jH,iH]
    yH = YH[jH,iH]
    x0 = XA[ii]
...
# Coordinates in the reference square:
  ...
  plt.ion()
  figr2 = plt.figure(1,figsize=(7,7), constrained_layout=False)
  fig2.clf()
