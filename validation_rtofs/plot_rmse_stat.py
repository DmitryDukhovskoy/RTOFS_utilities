...
ax11 = fig1.add_axes([0.1, 0.55, 0.8, 0.4])
im11 = ax11.pcolormesh(RMSE_mn, 
                       cmap=clrmp1,
                       vmin=0, vmax=0.25)
ax11.contour(Lmsk, [0.1], colors=[(0.,0.,0.)])
ax11.axis('scaled')
ax11.set_xlim([xl1,xl2])
ax11.set_ylim([yl1,yl2])
ax11.set_title('{0} mean RMSE(m) {3}, {1}-{2}'.format(expt,rdateS,rdateE,regn_nm))

# Colorbar
cax = fig1.add_axes([0.9, 0.55, 0.015, 0.4])
...
# Plot production:
ax12 = fig1.add_axes([0.1, 0.07, 0.8, 0.4])
im12 = ax12.pcolormesh(RMSEp_mn,
                     cmap=clrmp1,
                     vmin=0, vmax=0.25)

ax12.contour(Lmsk, [0.1], colors=[(0.,0.,0.)])
ax12.axis('scaled')
ax12.set_xlim([xl1,xl2])
ax12.set_ylim([yl1,yl2])
ax12.set_title('Product mean RMSE(m) {2}, {0}-{1}'.format(rdateS,rdateE,regn_nm))

# Colorbar
cax2 = fig1.add_axes([0.9, 0.07, 0.015, 0.4])
...
ax21.set_title('{0} & Product RMSE(m) {3}, {1}-{2}'.\
                format(expt,rdateS,rdateE,regn_nm))
