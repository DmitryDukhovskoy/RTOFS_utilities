"""
  For forecast skill assessment
  compute saturation MHD score
  see Dukhovskoy et al., 2023
  
  Use random fields to derive saturation MHD
...
rdateS = '20230303'
rdateE = '20230531'
dltD   = 30 # min # of days from the forecast considered as random field
Rlast  = '20230710'

sfx     = 'n-24'

# Figure output directory:
f_save = 1   # =0 - not save, =1 rewrite/save, =2 - cotinue from saved
...
dnmbLast = int(mtime.rdate2datenum(Rlast))
dnmbFrst = int(mtime.rdate2datenum('20230104'))

flnmout  = 'MHD_saturation_value.pkl' 
...
for dnmb0 in range(dnmbS,dnmbE+1):
  ...
  # randomly select  # of days around rdate to look for NAVO
  # fronts using missing=... 
  importlib.reload(mrtofs)
  importlib.reload(mgulf)
  pthnavo = '/scratch2/NCEPDEV/ovp/Lichuan.Chen/DCOM/'
  dD  = random.randrange(dltD,100) 
  sgn = random.randrange(1,3)
  if sgn == 1:
    sgn = -1
  else:
    sgn = 1
  ...
  if dD < dnmbFrst:
    dnmb_navo = dnmbFrst
  rdate_navo = mtime.dnumb2rdate(dnmb_navo, ihours=False) 

  XNW_navo, YNW_navo, navonmb = mgulf.read_navogs(rdate_navo, pthnavo, missing=5)
  if navonmb < 0.:
...
# Compute MHD
  ...
  print('=== Random {0} vs NEMO Gulf Stream MHD = {1:7.2f}km\n'.format(expt, mhdGS))
