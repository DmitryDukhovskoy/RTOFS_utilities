"""
  Script has not been tested / checked 
  !!!
...
parser.add_argument("--syrm",  
             help="GDAS start year/month YYYYMM to interpolate onto mesh025 and combine by months", 
             type=int,
             required=True)
parser.add_argument("--eyrm",  
             help="GDAS end year/month YYYYMM to interpolate onto mesh025 and combine by months", 
             type=int,
             required=True)
...
parser.add_argument("--tmpf",  
                    help=f">0: Save, start from last processed daily field every tmpf processed days, default=0", 
                    default=0,
                    type=int)
...
dnmbE = mtime.datenum([YRe,MMe,mtime.month_days(MMe,YRe)])
...
def write_nc(dfliceout, time_dnmb, A3d):
  ...
  dset.attrs['source']      = 'interp_GDAS_T2m_daily_mesh025.py'
...
for iday, dnmb0 in enumerate(DNMB):
  ...
  if not MM == MMw:
    ...
    if irec > 0:
      ...
      if not fsave:
        ...
        flt2m_out = f"GDAS_t2m_interp_mesh025_{YRw}{MMw:02d}_{regn}.nc"
  ...
  if save_tmp and not tmp_loaded:
    tmp_file = os.path.join(tmp_dir, f'GDAS_T2m_mesh025_{YR}{MM:02d}_{regn}.npz')
    tmp_loaded = True   # try to load tmp file only once
    if os.path.isfile(tmp_file):
      print(f"Loading tmp file: {tmp_file}")
      tmp = np.load(tmp_file)
      A3d = tmp["A3d"].tolist()
      time_dnmb = tmp["TM"].tolist()
      irec = len(time_dnmb)
      continue

  if len(time_dnmb) > 0 and dnmb0 <= time_dnmb[-1]:
    print(f"Skipping {YR}/{MM}/{DD}, processed ...")
    continue
  ...
  if save_tmp and irec%tmpf == 0:
    print(f"Saving tmp file --> {tmp_file}")
    np.savez(tmp_file, A3d=np.asarray(A3d), TM=np.asarray(time_dnmb))
...
if iday > 0:
  if not save_nc:
    ...
    flt2m_out = f"GDAS_t2m_interp_mesh025_{YRw}{MMw:02d}_{regn}.nc"
    pthintrp = pths_ml["GDAS"]["pthit2m"].format(regn=regn, YR=YRw)
