"""
  JEDI SOCA analysis ice  thickness fields 
  May need to get data from HPSS first
  Use:
  sbatch get_soca_ice_restart.sh --rdate 20250401
...
for iplt, rdate in enumerate(RDATES):
  ...
  pthice    = pths_ml["GDAS"]["pthsoca_ice"].format(YR=YR, MM=MM, DD=DD)
  flice = pths_ml["GDAS"]["flsoca_ice"].format(YR=YR, MM=MM, DD=DD)
  dflice = os.path.join(pthice, flice)
  print(f"Reading {YR}/{MM}/{DD}: {dflice}")
  ...
  sttl = f"ithkn SOCA CICE6\n{YR}/{MM:02d}/{DD:02d}"

  irow = iplt // ncol
  icol = iplt % ncol
  ax1 = axes[irow, icol]

  img = plot_field(ax1, m, xh, yh, vice_cell, clrmp, rmin, rmax, regn, sttl)
...
btx = f' @{machine}: plot_SOCAcice_ithkn_Nsbplt.py'
