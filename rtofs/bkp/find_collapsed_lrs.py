...
Dan's directory rtofs.20221117 contains the initial forecast run. We can not start assimilating directly from the NAVY restart
since we don't have either the previous 6 hours restarts for incremental update nor the surface archives to do fgat. When
we use a navy restart we run a 36 (or 48) hour forecast to have the needed files. That is what you are seeing in Dan's rtofs.20221117,
forecast only.
...
flnm = 'archv_1_inc.2022_321_00'    # add increments from ncoda_archv_inc
...
#  drfnm = "rtofs."+rdate+"/"+flnm
  ...
  ZZs, Hb, XX, dZZs = psct.plot_Jsection(xsct,ZZ,HH,LON,LAT,drfnm,sct_show=ax1,\
                      fgnmb=5,dcntr=1, zstart=-1000., jlrs=jp0)
#  print_1D(dZZs[:,jp0])
