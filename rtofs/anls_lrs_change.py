...
f_cont = True      # continue from last saved
...
RMMX = []
if f_cont:
  print('Continue from last saved record')
  print('Loading '+fldump)
  with open(fldump,'rb') as fid:
    RMMX  = pickle.load(fid)
    TM    = RMMX.time
    dnmbS = TM[-1]  # start from last saved to update old Tprf, Sprf
    rdateS = mtime.dnumb2rdate(dnmbS, ihours=False)
    print('Last saved: ' + rdateS)
...
#  pthpara = '/scratch2/NCEPDEV/marine/Zulema.Garraffo/wcoss2.paraD1' \
  pthpara = '/scratch2/NCEPDEV/marine/Dmitry.Dukhovskoy/data/paraD/'\
            + '/rtofs.' + rdate0 + '/'
